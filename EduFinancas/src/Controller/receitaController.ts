...
//     try {
//       const receita = await this.receitaService.getReceitaByIdWithUser(receitaId);
//       if (!receita) {
//         return res.status(404).json({ message: "Receita não encontrada" });
//       }
  
//       // Verifica se a receita pertence ao usuário autenticado
//       if (!req.user || receita.user.toString() !== req.user.id) {
//         return res.status(403).json({ message: "Acesso não autorizado a esta receita" });
//       }
